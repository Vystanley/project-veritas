...
**Every day, millions of people scroll through TikTok, Instagram Reels, and YouTube Shorts. Some of those videos contain misinformation that spreads faster than anyone can debunk it. Veritas was built to change that.**

Paste a video link. In under 2 minutes, Veritas downloads the video, transcribes what's being said, reads what's shown on screen, searches the web for real sources, checks for recycled footage, runs a deepfake scan, and delivers a verdict with per-claim citations — so you can see *exactly* why something is true, false, or somewhere in between.
...
*The backend runs on Render's free tier, so the first scan may take an extra 30-60 seconds while the server wakes up. After that, scans typically complete in 1-2 minutes.*
...
7. **AI fact-check** — Claude Sonnet analyzes the transcript + visual content + web sources and produces a structured verdict with per-claim explanations and citations.

All of these steps run in parallel where possible. Source citations are gated — the AI can only cite URLs that came back from real web searches. Fabricated citations are automatically detected, dropped, and logged.

---

## Why I Built This

I'm a student who noticed that fact-checking content is almost always focused on text articles and tweets. Short-form video — the format that actually dominates how young people consume news — gets almost no automated fact-checking coverage.
...
Veritas is my attempt to fill that gap. It's not perfect — no AI fact-checker is — but it demonstrates that the pipeline is technically feasible and practically useful, even on completely free infrastructure.
...
The project includes an evaluation harness (`backend/evals/`) with 15 test cases spanning true claims, false claims, partially true content, and control cases. Each test case has an expected verdict and is scored on two dimensions:
...
This lets me measure whether code changes improve or regress fact-checking quality.
...
- **Speed depends on free infrastructure** — Render's free tier sleeps after 15 minutes of inactivity. First scan after wake-up is slowest.
...
The hardest part wasn't the AI — it was making the whole thing fast enough to be usable without spending any money.
